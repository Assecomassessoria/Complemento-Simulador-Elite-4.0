...
export function calculateSimulation(state: SimulatorState): CalculatedValues {
  ...
  // D32: Primeiras Mensais = ((8% do Imóvel) - Ato) / Qtd Primeiras Mensais
  // Nota: Primeiras Parcelas são calculadas para atingir 8% de entrada (Ato + Primeiras)
  const primeirasMensais = ((state.valorImovel * 0.08) - state.valorAto) / state.qtdPrimeirasMensais;
  ...
  // ===== VALIDAÇÃO =====
  // F11: Validação OK Entrada = SE((Ato + (Primeiras Mensais * Qtd Primeiras)) >= (8% do Imóvel); "OK ENTRADA"; "VERIFICAR")
  const entradaTotal = state.valorAto + (state.qtdPrimeirasMensais * primeirasMensais);
  const minimoExigido = state.valorImovel * 0.08;
  const validacaoEntrada = entradaTotal >= minimoExigido ? "OK ENTRADA" : "VERIFICAR";
